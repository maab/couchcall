...
exports.init = function(newDoc, oldDoc, userCtx, secObj) {
  ...
  v.notMatches = function(field, regex, message) {
    if (newDoc[field].match(regex)) {
      message = message || "Format of '"+field+"' field is invalid.";
      v.forbidden(message);    
    }
    ...
    v.notEmpty = function(){
        for (var i=0; i < arguments.length; i++) {
            ...
            if (newDoc[field].length == 0 ) v.forbidden(message); 
        };
    };
    
    v.hasField = function(field){
        return typeof newDoc[field] == "undefined" ? false : true;
    }
